...
def model(
    ...
    # measurement times
    ts = jnp.arange(0, 160+1., 1.)
    # parameters alpha, beta, gamma, delta of dz_dt
    theta = numpyro.sample(
        "theta",
        dist.LogNormal(loc=jnp.array([jnp.log(loc) for loc in prior_params["loc"]]),
    ...
    for i in range(n_obs):
        if x_obs is None:
            numpyro.sample(
                f"obs_{i}",
                dist.Binomial(total_count=1000, probs=jnp.clip(z_sub[:, 1]/N, 0.0, 1.0))
            )
        else:
            numpyro.sample(
                f"obs_{i}",
                dist.Binomial(total_count=1000, probs=jnp.clip(z_sub[:, 1]/N, 0.0, 1.0)), obs=x_obs[i]
            )
...
class SIR():
    ...
    def prior(self, torch_version=None):
        ...
        if torch_version:
            return torch.distributions.LogNormal(
                loc=torch.tensor([torch.log(loc) for loc in self.prior_params["loc"]]),
                scale=torch.tensor(self.prior_params["scale"])
            )
        else:
            return dist.LogNormal(
                loc=jnp.array([jnp.log(loc) for loc in self.prior_params["loc"]]),
    ...
    def simulator(self, rng_key, theta, n_obs=1, torch_version=None):
        ...
        data = predictive(subkey, n_obs=n_obs)
    ...
    def generate_training_data(self, rng_key, n_simulations, save=True):
        ...
        if self.torch_version:
            ...
            theta_train = torch.from_numpy(np.asarray(theta_train)).float()
    ...
    def generate_reference_data(self, rng_key, nb_obs=25, n_repeat=100, save=True, load_theta=False):
        ...
        if not load_theta:
            ...
            if self.torch_version:
                theta_star = torch.from_numpy(np.asarray(theta_star)).float()
            if save:
                print('Saving at', filename)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path', type=str, default='data/sir/', help='Path to save the data')

    args = parser.parse_args()

    rng_key = random.PRNGKey(1)

    sir = SIR(torch_version=True, save_path=args.save_path)
    os.makedirs(sir.save_path, exist_ok=True)

    if args.train_data:
        sir.generate_training_data(rng_key, n_simulations=50_000)

    if args.reference_data:
        sir.generate_reference_data(rng_key)
    ...
    if args.reference_posterior:
        ...
        for num_obs in num_obs_list:
            for n_obs in n_obs_list:
                sir.generate_reference_posterior_samples(rng_key, num_obs, n_obs, num_samples=1000)
