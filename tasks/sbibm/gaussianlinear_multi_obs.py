...
class GaussianLinear_multiobs(GaussianLinear):
    def __init__(
        self, dim=10, prior_scale=1, simulator_scale=torch.linspace(0.6, 1.4, 10)
    ):
...
if __name__ == "__main__":
    ...
    task = GaussianLinear_multiobs(
        dim=10, prior_scale=torch.ones(10), simulator_scale=torch.linspace(0.6, 1.4, 10)
    )
    ...
    for n_repetitions in [1, 10, 50]:
        ...
        posterior_samples[n_repetitions] = task._sample_reference_posterior_multiobs(
            num_samples=10_000, observation_list=xs
        )
    ...
    for n_repetitions in [1, 10, 50]:
        ...
        sns.kdeplot(
            posterior_samples[n_repetitions].numpy()[:, 0],
            bw_method=0.5,
            lw=2.0,
            ax=ax,
            label=n_repetitions,
