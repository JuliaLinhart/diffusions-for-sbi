...
class GausianLinear_multiobs(GaussianLinear):
    def __init__(
        self,
        dim=10,
        prior_scale=1,
    ...
    def _get_reference_posterior_multiobs(self, observation_list):
        """Gets posterior for the case with multiple observations

        Args:
            observation_list: list of observations to condition on
        ...
        loc = torch.matmul(
            ...
            (
                ...
                * torch.matmul(
                    ...
                    torch.mean(observation_list, dim=0).reshape(-1)
                )
                + torch.matmul(
                    self.prior_params["precision_matrix"],
    ...
    def _sample_reference_posterior_multiobs(
            ...
            num_samples,
            observation_list):
        posterior = self._get_reference_posterior_multiobs(observation_list)
...
if __name__ == "__main__":
    ...
    task = GausianLinear_multiobs(dim=10,
