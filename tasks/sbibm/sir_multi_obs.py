...
def model(x_obs=None, n_obs=1):
    ...
    theta = numpyro.sample(
        "theta",
        dist.LogNormal(loc=jnp.array([jnp.log(0.4), jnp.log(0.8)]),
                       scale=jnp.array([0.50, 0.20]))
    )
    ...
    for i in range(n_obs):
        if x_obs is None:
            ...
            numpyro.sample(
                f"obs_{i}",
                dist.Binomial(total_count=1000, probs=z[:, 1]/N), obs=x_obs[i]
            )
...
if __name__ == '__main__':
    ...
    for num_obs in range(1, 10+1):
        ...
        for n_obs in [1, 8, 14, 22, 30]:
            ...
            filename = f'./samples-plcr/true_posterior_samples_num_{num_obs}_n_obs_{n_obs}.pkl'
    ...
    prior = dist.LogNormal(
        loc=jnp.array([jnp.log(0.4), jnp.log(0.125)]),
        scale=jnp.array([0.50, 0.20])
    )
