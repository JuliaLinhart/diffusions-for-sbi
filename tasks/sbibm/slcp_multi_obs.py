...
class SLCP_multiobs(SLCP):
    ...
    def _sample_reference_posterior_multiobs(self, num_samples, x_obs):
        n_obs = x_obs.shape[0]
        data = {
            f"obs_{i}": jnp.array(self.unflatten_data(x_obs[i, :]).unsqueeze(0))
            for i in range(n_obs)
        }
        kernel = NUTS(self.model, init_strategy=init_to_uniform(None, radius=3))
        mcmc = MCMC(kernel, num_warmup=1000, num_samples=num_samples, num_chains=4)
        mcmc.run(rng_key=rng_key, x_obs=data, n_obs=n_obs)
...
if __name__ == "__main__":
    ...
    for n_obs in n_obs_list:
        ...
        data = {
            f"obs_{i}": jnp.array(x_obs_100[i, :].unsqueeze(0)) for i in range(n_obs)
        }
        print(data[f"obs_0"].shape)
        kernel = NUTS(model, init_strategy=init_to_uniform(None, radius=3))
        mcmc = MCMC(kernel, num_warmup=100, num_samples=2500, num_chains=4)
        mcmc.run(rng_key, x_obs=data, n_obs=n_obs)
