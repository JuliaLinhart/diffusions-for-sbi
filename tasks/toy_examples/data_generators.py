...
class Gaussian_MixtGaussian_mD:
    def __init__(self, dim, rho_min=.6, rho_max=1.4, device='cuda:0') -> None:
        """
        Prior: mD Gaussian: theta ~ N(means, diag(scales)).
        Simulator: mD Gaussian: x ~ N(theta, rho * I_m).
        SBI task: infer theta from x.
        """
        self.dim = dim
        self.simulator_base_std = torch.linspace(rho_min, rho_max, dim).to(device)**.5
        self.prior = torch.distributions.MultivariateNormal(
    ...
    def simulator(self, theta):
        cp_dist = MultivariateNormal(loc=theta[None].repeat(2, 1),
                                    scale_tril=torch.stack((torch.diag(self.simulator_base_std*(2.25)**.5),
                                                          torch.diag(self.simulator_base_std/3)
                                                          ),
                                                         dim=0))
        samples_x = torch.distributions.MixtureSameFamily(
            component_distribution=cp_dist,
            mixture_distribution=torch.distributions.Categorical(probs=torch.ones(2).to(self.device)/2)).sample((1,))[0]
        return samples_x
    ...
    def true_posterior(self, x_obs):
        ...
        log_weights = (torch.distributions.Normal(loc=torch.zeros_like(x_obs),
                                                  scale=(2.25**.5)*self.simulator_base_std + 1).log_prob(x_obs).sum(dim=-1),
                       torch.distributions.Normal(loc=torch.zeros_like(x_obs),
                                                  scale=(1/3) * self.simulator_base_std + 1).log_prob(x_obs).sum(dim=-1),
                       )

        base_comp = MultivariateNormal(
            loc=torch.stack((equivalent_post_diag_cov_1 * x_obs / ((self.simulator_base_std**2)*2.25),
                             equivalent_post_diag_cov_2 * x_obs  / ((self.simulator_base_std**2)*(1/9))), dim=0),
            scale_tril=torch.stack((
                torch.diag(equivalent_post_diag_cov_1**.5),
                torch.diag(equivalent_post_diag_cov_2**.5)
            ), dim=0))
        return torch.distributions.MixtureSameFamily(
            component_distribution=base_comp,
            mixture_distribution=torch.distributions.Categorical(logits=torch.stack(log_weights))
        )
    ...
    def diffused_posterior(self, x_obs, alpha_t):
        ...
        return torch.distributions.MixtureSameFamily(
            component_distribution=MultivariateNormal(
                loc=posterior_0.component_distribution.loc*(alpha_t**.5),
                covariance_matrix=cov * alpha_t + (1-alpha_t)*torch.eye(cov.shape[-1])[None].to(cov.device)),
