...
if __name__ == "__main__":
    ...
    fig, axes_all = plt.subplots(n_rows, n_plots, sharex=True, sharey=True, squeeze=False, figsize=(5 * n_plots, 5*n_rows)) #, constrained_layout=True)
    # fig, axes_all = plt.subplots(n_rows, n_plots, sharex=True, sharey=True, squeeze=False, figsize=(1 + 4 * n_plots, 1 + 1.5*n_rows))
    fig.subplots_adjust(right=.995, top=.95, bottom=.2, hspace=0, wspace=0, left=.1)
    ...
    #remove some dims
    equivalent_speed_data_small = equivalent_speed_data.loc[equivalent_speed_data.dim.isin(equivalent_speed_data.dim.unique()[dim_idxs])]

    # Group data per dim (row)
    for axes, (dim, dim_data) in zip(axes_all, equivalent_speed_data_small.groupby(['dim'])):
        ...
        for (eps, eps_data), ax in zip(dim_data.groupby(['eps']), axes):
            ...
            for (alg, sampling_steps), dt in eps_data.groupby(['alg', "sampling_steps"]):
                ...
                ax.plot(dt.groupby(['N_OBS']).first().index,
                        dt.groupby(['N_OBS']).sw_norm.mean(),
                        **plot_kwars)
                ax.fill_between(dt.groupby(['N_OBS']).first().index,
                                dt.groupby(['N_OBS']).sw_norm.mean() - 1.96*dt.groupby(['N_OBS']).sw_norm.std() / (dt.groupby(['N_OBS'])['seed'].count()**.5),
                                dt.groupby(['N_OBS']).sw_norm.mean() + 1.96*dt.groupby(['N_OBS']).sw_norm.std() / (dt.groupby(['N_OBS'])['seed'].count()**.5),
                                alpha=alpha_fill, color=plot_kwars['color'])
    ...
    n_rows = len(dim_idxs)
    fig, axes = plt.subplots(n_rows, n_cols, sharex=True, sharey=True, squeeze=False, figsize=(5* n_cols, 5*n_rows)) #, constrained_layout=True)
    fig.subplots_adjust(right=.995, top=.95, bottom=.2, hspace=0, wspace=0, left=.15)
    ...
    for ax, ((dim, alg), alg_data) in zip(axes.flatten(), equivalent_speed_data_small.groupby(['dim', 'alg'])):
        if alg == "Langevin":
            alg = "LANGEVIN"
    ...
    for ax, ((alg, dim), alg_data) in zip(axes.flatten(), equivalent_speed_data_small.groupby(['alg', 'dim'])):
