...
def load_samples(task_name, n_train, lr, n_obs, cov_mode=None, langevin=False):
    ...
    for num_obs in NUM_OBSERVATION_LIST:
        if langevin:
            filename = (
                PATH_EXPERIMENT
                + f"{task_name}/n_train_{n_train}_n_epochs_1000_lr_{lr}/langevin_steps_400_5/posterior_samples_{num_obs}_n_obs_{n_obs}.pkl"
            )
        else:
            filename = (
                PATH_EXPERIMENT
                + f"{task_name}/n_train_{n_train}_n_epochs_1000_lr_{lr}/euler_steps_1000/posterior_samples_{num_obs}_n_obs_{n_obs}_{cov_mode}.pkl"
            )
...
def compute_mean_distance_to_true_theta(
    task_name, n_train, n_obs, cov_mode=None, langevin=False, percentage=0
):
    ...
    # outliers
    outliers = [
        count_outliers(samples[num_obs], task.get_true_parameters(num_obs))
        for num_obs in NUM_OBSERVATION_LIST
    ]
    outlier_dict = {
        "mean": torch.tensor(outliers).mean().item(),
        "std": torch.tensor(outliers).std().item(),
    }
    # compute dist between samples and true theta
    dist_dict = {"mmd": [], "mse": []}
    for num_obs in NUM_OBSERVATION_LIST:
        dist = dist_to_dirac(
            samples[num_obs], task.get_true_parameters(num_obs), percentage=percentage
        )
...
if __name__ == "__main__":
            ...
            for j, n_train in enumerate(N_TRAIN):
                for k, lr in enumerate(LR_LIST):
                    train_losses, val_losses = load_losses(
                        task_name, n_train, lr, path=PATH_EXPERIMENT
                    )
        ...
        for metric in METRICS:
            ...
            for i, task_name in enumerate(TASKS):
                ...
                for j, n_train in tqdm(
                    ...
                    for n_obs in N_OBS:
                        # langevin
                        dist_dict, outlier_dict = compute_mean_distance_to_true_theta(
                            task_name, n_train=n_train, n_obs=n_obs, langevin=True
                        )
                        ...
                        # gauss
                        dist_dict, outlier_dict = compute_mean_distance_to_true_theta(
                            task_name, n_train=n_train, n_obs=n_obs, cov_mode="GAUSS"
                        )
                    ...
                    mean_dist_l, std_dist_l = torch.FloatTensor(
                        mean_dist_l
                    ), torch.FloatTensor(std_dist_l)
                    mean_dist_g, std_dist_g = torch.FloatTensor(
                        mean_dist_g
                    ), torch.FloatTensor(std_dist_g)
                    # mean_dist_j, std_dist_j = torch.FloatTensor(mean_dist_j), torch.FloatTensor(std_dist_j)
                    axs[i, j].fill_between(
                        N_OBS,
                        mean_dist_l - std_dist_l,
                        mean_dist_l + std_dist_l,
                        alpha=0.2,
                    )
                    axs[i, j].plot(
                        N_OBS,
                        mean_dist_l,
                        label=f"{metric} (langevin)",
                        linewidth=3,
                        marker="o",
                    )
                    axs[i, j].fill_between(
                        N_OBS,
                        mean_dist_g - std_dist_g,
                        mean_dist_g + std_dist_g,
                        alpha=0.2,
                    )
                    axs[i, j].plot(
    ...
    if args.dist_ntrain:
        ...
        for percentage in PERCENTAGES:
            ...
            for metric in METRICS:
                    ...
                    for j, n_obs in tqdm(
                        ...
                        for n_train in N_TRAIN:
                            ...
                            (
                                dist_dict,
                                outlier_dict,
                            ) = compute_mean_distance_to_true_theta(
                                task_name,
                                n_train=n_train,
                                n_obs=n_obs,
                                langevin=True,
                ...
                plt.savefig(
                    PATH_EXPERIMENT
                    + f"{metric}_distance_to_true_theta_n_train_percentage_{percentage}.png"
                )
                plt.savefig(
                    PATH_EXPERIMENT
                    + f"{metric}_distance_to_true_theta_n_train_percentage_{percentage}.pdf"
                )
                plt.clf()
    ...
    if args.corner_plots:
        ...
        for task_name in TASKS:
            true_thetas = (
                get_task(task_name).get_true_parameters(num_obs).repeat(1000, 1)
            )
            ...
            for n_train in [30000]:
                ...
                for n_obs in N_OBS:
                    samples.append(
                        load_samples(
                            task_name, n_train, lr=LR, n_obs=n_obs, cov_mode=cov_mode
                        )[num_obs]
                    )
                ...
                multi_corner_plots(
                    samples,
                    legends=[f"n_obs={n_obs}" for n_obs in N_OBS] + ["true theta"],
                    colors=["blue", "orange", "green", "red", "purple", "black"],
                    title=f"{task_name}, n_train={n_train}, num_obs={num_obs}",
                )
                plt.savefig(
                    PATH_EXPERIMENT
        ...
        for task_name in TASKS:
            ...
            for n_train in [30000]:
                ...
                for n_obs in N_OBS:
                    samples.append(
                        load_samples(
                            task_name, n_train, lr=LR, n_obs=n_obs, langevin=True
                        )[num_obs]
                    )
                multi_corner_plots(
                    samples.append(true_theta),
                    legends=[f"n_obs={n_obs}" for n_obs in N_OBS] + ["true theta"],
                    colors=["blue", "orange", "green", "red", "purple", "black"],
                    title=f"{task_name}, n_train={n_train}, num_obs={num_obs}",
                )
                plt.savefig(
                    PATH_EXPERIMENT
                    + f"corner_plots_{task_name}_n_train_{n_train}_num_obs_{num_obs}_langevin.png"
                )
                plt.savefig(
                    PATH_EXPERIMENT
                    + f"corner_plots_{task_name}_n_train_{n_train}_num_obs_{num_obs}_langevin.pdf"
                )
                plt.clf()
        ...
        for i, task_name in enumerate(TASKS):
            ...
            for j, n_train in tqdm(enumerate(N_TRAIN), desc=f"{task_name}"):
                ...
                for k, n_obs in enumerate(N_OBS):
                    true_parameters = [
                        task.get_true_parameters(num_obs)
                        for num_obs in NUM_OBSERVATION_LIST
                    ]
                    # langevin
                    samples = load_samples(
                    ...
                    outliers = [
                        count_outliers(samples[num_obs], true_parameters[num_obs - 1])
                        for num_obs in NUM_OBSERVATION_LIST
                    ]
                    ...
                    # gauss
                    samples = load_samples(
                        task_name, n_train, lr=LR, n_obs=n_obs, cov_mode="GAUSS"
                    )
                    outliers = [
                        count_outliers(samples[num_obs], true_parameters[num_obs - 1])
                        for num_obs in NUM_OBSERVATION_LIST
                    ...
                    # # jac
                    # if task_name in ["sir", "lotka_volterra"]:
                    #     samples = load_samples(task_name, n_train, lr=LR, n_obs=n_obs, cov_mode="JAC")
                ...
                outliers_mean_langevin, outliers_std_langevin = torch.FloatTensor(
                    outliers_mean_langevin
                ), torch.FloatTensor(outliers_std_langevin)
                outliers_mean_gauss, outliers_std_gauss = torch.FloatTensor(
                    outliers_mean_gauss
                ), torch.FloatTensor(outliers_std_gauss)
                # outliers_mean_jac, outliers_std_jac = torch.FloatTensor(outliers_mean_jac), torch.FloatTensor(outliers_std_jac)
                axs[i, j].fill_between(
                    N_OBS,
                    outliers_mean_langevin - outliers_std_langevin,
                    outliers_mean_langevin + outliers_std_langevin,
                    alpha=0.2,
                )
                axs[i, j].plot(
                ...
                axs[i, j].fill_between(
                    N_OBS,
                    outliers_mean_gauss - outliers_std_gauss,
                    outliers_mean_gauss + outliers_std_gauss,
                    alpha=0.2,
                )
                axs[i, j].plot(
                    N_OBS, outliers_mean_gauss, label=f"GAUSS", linewidth=3, marker="o"
                )
                # if task_name in ["sir", "lotka_volterra"]:
