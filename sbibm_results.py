...
from plot_utils import multi_corner_plots
...
def load_samples(task_name, n_train, lr, n_obs, cov_mode=None, langevin=False):
    ...
    for num_obs in NUM_OBSERVATION_LIST:
        if langevin:
            filename = EXPERIMENT_PATH + f'{task_name}/n_train_{n_train}_n_epochs_1000_lr_{lr}/langevin_steps_400_5/posterior_samples_{num_obs}_n_obs_{n_obs}.pkl'
        else:
            filename = EXPERIMENT_PATH + f'{task_name}/n_train_{n_train}_n_epochs_1000_lr_{lr}/euler_steps_1000/posterior_samples_{num_obs}_n_obs_{n_obs}_{cov_mode}.pkl'
        samples[num_obs] = torch.load(filename)
    return samples

# compute mean distance to true theta over all observations 
def compute_mean_distance_to_true_theta(task_name, n_train, n_obs, cov_mode=None, langevin=False):
    ...
    dist_dict = {"mmd": [], "mse": []}
    for num_obs in NUM_OBSERVATION_LIST:
        dist = dist_to_dirac(samples[num_obs], task.get_true_parameters(num_obs))
        ...
        for metric in METRICS:
            ...
            for i, task_name in enumerate(TASKS):
                ...
                for j,n_train in tqdm(enumerate(N_TRAIN), desc=f"{task_name}, {metric}"):
                    ...
                    axs[i, j].legend()
            plt.savefig(EXPERIMENT_PATH + f"{metric}_distance_to_true_theta_n_obs.png")
            plt.savefig(EXPERIMENT_PATH + f"{metric}_distance_to_true_theta_n_obs.pdf")
            plt.clf()
        ...
        for metric in METRICS:
            ...
            for i, task_name in enumerate(TASKS):
                ...
                for j,n_obs in tqdm(enumerate(N_OBS), desc=f"{task_name}, {metric}"):
                    ...
                    axs[i, j].legend()
            plt.savefig(EXPERIMENT_PATH + f"{metric}_distance_to_true_theta_n_train.png")
            plt.savefig(EXPERIMENT_PATH + f"{metric}_distance_to_true_theta_n_train.pdf")
            plt.clf()
    ...
    if args.corner_plots:
        ...
        for task_name in TASKS:
            ...
            for n_train in [30000]:
                ...
                plt.savefig(EXPERIMENT_PATH + f"corner_plots_{task_name}_n_train_{n_train}_num_obs_{num_obs}.png")
                plt.savefig(EXPERIMENT_PATH + f"corner_plots_{task_name}_n_train_{n_train}_num_obs_{num_obs}.pdf")
                plt.clf()
        ...
        for task_name in TASKS:
            ...
            for n_train in [30000]:
                ...
                plt.savefig(EXPERIMENT_PATH + f"corner_plots_{task_name}_n_train_{n_train}_num_obs_{num_obs}_langevin.png")
                plt.savefig(EXPERIMENT_PATH + f"corner_plots_{task_name}_n_train_{n_train}_num_obs_{num_obs}_langevin.pdf")
                plt.clf()
    ...
    if args.outliers:
        ...
        for i, task_name in enumerate(TASKS):
            ...
            for j, n_train in tqdm(enumerate(N_TRAIN), desc=f"{task_name}"):
                ...
                axs[i, j].set_xticks(N_OBS)
                axs[i, j].legend()
        plt.savefig(EXPERIMENT_PATH + f"outliers_nobs.png")
        plt.savefig(EXPERIMENT_PATH + f"outliers_nobs.pdf")
        plt.clf()
        ...
        for i, task_name in enumerate(TASKS):
            ...
            for j, n_obs in tqdm(enumerate(N_OBS), desc=f"{task_name}"):
                ...
                axs[i, j].set_xscale("log")
                axs[i, j].legend()
        plt.savefig(EXPERIMENT_PATH + f"outliers_n_train.png")
        plt.savefig(EXPERIMENT_PATH + f"outliers_n_train.pdf")
        plt.clf()
