...
def get_hmc_samples(task, prior, x_obs_100, N_OBS):
    ...
    hmc = MCMC(
        ...
        warmup_steps=500,
        initial_params=
        {"theta": torch.randn((10_000, DIM)).to(device) + x_obs[:N_OBS].mean()}
    )
    hmc.run()
...
if __name__ == "__main__":
    ...
    from nse_toy import NSE, NSELoss
    ...
    for DIM in [2, 4, 8, 10, 16, 32, 64]:
        for eps in [0, 1e-3, 1e-2, 1e-1]:  
            for seed in range(5):
                ...
                prior_score_fn = get_vpdiff_gaussian_score(prior.loc.to(device), prior.covariance_matrix.to(device), score_net)

                def prior_score(theta, t):
                    mean_prior_0_t = mean_backward(theta, t, prior_score_fn, score_net)
                    prec_prior_0_t = prec_matrix_backward(t, prior.covariance_matrix.to(device), score_net).repeat(
                        theta.shape[0], 1, 1
                    )
                ...
                for N_OBS in [2, 4, 8, 16, 32, 64, 90][::-1]:
                    ref_samples = MALA(
                        x=torch.randn((10_000, DIM)).to(device)*(1 / N_OBS) + x_obs[:N_OBS].mean(),
                        lr=1e-3 / N_OBS,
                    ...
                    for sampling_steps, eta in zip(
                        ...
                        samples_ddim = score_net.ddim(shape=(1000*N_OBS,),
                                                      x=x_obs_100[None, :N_OBS].repeat(1000, 1, 1).reshape(1000*N_OBS, -1).to(device),
                                                      steps=100,
                        ...
                        samples_gauss = score_net.ddim(shape=(1000,),
                                                       x=x_obs_100[:N_OBS].to(device),
                                                       eta=eta,
                                                       steps=sampling_steps,
                                                       prior_score_fn=prior_score,
                                                       dist_cov_est=cov_est.to(device),
                                                       cov_mode='GAUSS').cpu()
                        ...
                        dt_lang = t_end_lang - tstart_lang
                            ...
                            # true_posterior_mean = (true_posterior_cov @ (inv_prior_prior + inv_lik @ x_obs_100[:N_OBS].sum(dim=0).to(device)))
