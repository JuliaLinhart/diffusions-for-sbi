...
def load_reference_samples(task_name, n_obs):
    ...
    for num_obs in NUM_OBSERVATION_LIST:
        ...
        samples_ref[num_obs] = samples_ref[num_obs][torch.randperm(samples_ref[num_obs].shape[0])]
...
if __name__ == "__main__":
                ...
                for n_train in N_TRAIN:
                    for n_obs in N_OBS:
                        for method in METHODS_STYLE.keys():
                            ...
                            dist_mean = dist["mean"]
                            dist_std = dist["std"]
                            print(f"{task_name}, {n_train}, {n_obs}, {method}: {dist_mean}, {dist_std}")
    ...
    if args.plot_dist:
        ...
        if args.swd or args.mmd or args.dirac or args.c2st:
            metric = "swd" if args.swd else "mmd" if args.mmd else "mmd_to_dirac" if args.dirac else "c2st"
