...
PATH_EXPERIMENT = "results/toy_models/gaussian_mixture"
...
if __name__ == '__main__':
    ...
    # Read all the data!
    all_data = pd.read_csv(f"{PATH_EXPERIMENT}/gaussian_mixture_exp_treated.csv").reset_index()
    ...
    DIM = 10
    equivalent_speed_data = equivalent_speed_data.loc[(equivalent_speed_data.dim == DIM) & (all_data.eps < 1)]

    #plotting per epsilon
    n_plots = len(equivalent_speed_data['eps'].unique())
    fig, axes_all = plt.subplots(len(equivalent_speed_data['dim'].unique()), n_plots, sharex=True, sharey=True, squeeze=False, figsize=(5 * n_plots, 5)) #, constrained_layout=True)
    ...
    fig.subplots_adjust(right=.995, top=.92, bottom=.2, hspace=0, wspace=0, left=.15)
